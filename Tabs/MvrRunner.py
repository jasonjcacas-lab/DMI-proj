...
def build_tab(parent):
    ...
    # Top section: Copy-Paste Mode toggle switch
    # Load saved copy-paste mode state
    copy_paste_mode_saved = ui_settings.get("copy_paste_mode", False)
    copy_paste_mode_var = tk.BooleanVar(value=copy_paste_mode_saved)
    copy_paste_frame = ttk.Frame(right)
    copy_paste_frame.grid(row=0, column=0, sticky="ew", padx=0, pady=(0, 5))
    
    # Create toggle switch (custom button that looks like a switch)
    def create_toggle_switch(parent, var, text, command):
        """Create a toggle switch button"""
        switch_frame = ttk.Frame(parent)
        switch_frame.pack(side="left")
        
        label = ttk.Label(switch_frame, text=text + ":")
        label.pack(side="left", padx=(0, 5))
        
        def update_switch_appearance():
            if var.get():
                switch_btn.config(text="ON", style="Accent.TButton")
            else:
                switch_btn.config(text="OFF", style="TButton")
        ...
        switch_btn = ttk.Button(switch_frame, text="OFF", width=5, command=toggle)
        switch_btn.pack(side="left")
        update_switch_appearance()
        
        return switch_btn, update_switch_appearance
    
    # Store copy buttons and field frames for mode switching
    copy_buttons = {}
    field_frames = {}
    
    # Store run button frame reference for toggling (will be set later)
    run_btn_frame_ref = [None]
    ...
    def toggle_copy_paste_mode():
        """Toggle between normal mode and copy-paste mode"""
        mode_enabled = copy_paste_mode_var.get()
        # Show/hide copy buttons for all fields
        for key, frame in field_frames.items():
            if key in copy_buttons:
                if mode_enabled:
                    copy_buttons[key].pack(side="right", padx=(5, 0))
                else:
                    copy_buttons[key].pack_forget()
        
        # Hide "Run MVR's" button when Copy-Paste Mode is ON
        if run_btn_frame_ref[0] is not None:
            if mode_enabled:
                run_btn_frame_ref[0].grid_remove()  # Hide the run button frame
            else:
                run_btn_frame_ref[0].grid()  # Show the run button frame
        
        # Save state
        ui_settings["copy_paste_mode"] = mode_enabled
        _save_ui_settings(ui_settings)
    
    # Create toggle switch
    toggle_switch_btn, update_switch_ui = create_toggle_switch(
        copy_paste_frame,
        copy_paste_mode_var,
        "Copy-Paste Mode",
        toggle_copy_paste_mode
    )
